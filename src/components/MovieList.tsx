 ...
 */
const MovieList: React.FC<MovieListProps> = ({ movies: propMovies, showLoadMore = true }) => {
  const [movies, setMovies] = useState<Movie[]>(propMovies || []);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState<boolean>(!propMovies);
  ...
  useEffect(() => {
    // Chỉ fetch từ API nếu không có movies được truyền vào qua props
    if (!propMovies) {
      fetchMovies(1);
    }
  }, [fetchMovies, propMovies]);
